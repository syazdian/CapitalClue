...
public class LocalDbRepository : ILocalDbRepository
{
    ...
    public LocalDbRepository(IDbContextFactory<StapleSourceContext> dbContextFactory, IStateContainer stateContainer, ILogger<LocalDbRepository> logger)
    {
    ...
    //public async Task<List<BellSourceDto>> GetTotalOnlyInBell()
    //{
    //    try
    //    {
    //        var wireless = (from b in totalBellRecords.Where(c => c.RebateType != null && c.Phone != null).ToList()
    //                        join s in totalStaplesRecords.Where(c => c.RebateType != null && c.Phone != null).ToList()
    //                        on new { phone = b.Phone, rebateType = b.RebateType } equals new { phone = s.Phone, rebateType = s.RebateType } into belljointable
    //                        from j in belljointable.DefaultIfEmpty()
    //                        where b.OrderNumber != null && b.Lob == "Wireless" && j == null
    //                        select b).ToList();

    //        var wireline = (from b in totalBellRecords.Where(c => c.OrderNumber != null && c.SubLob != null).ToList()
    //                        join s in totalStaplesRecords.Where(c => c.OrderNumber != null && c.SubLob != null).ToList()
    //                        on new { ordernumber = b.OrderNumber, sublob = b.SubLob } equals new { ordernumber = s.OrderNumber, sublob = s.SubLob } into belljointable
    //                        from j in belljointable.DefaultIfEmpty()
    //                        where b.OrderNumber != null && b.Lob != "Wireless" && j == null
    //                        select b).ToList();

    //        var query = wireless.Union(wireline).ToList();
    //        return query;
    ...
    //public async Task<List<StaplesSourceDto>> GetTotalOnlyInStaple()
    //{
    //    try
    //    {
    //        var wireless = (from s in totalStaplesRecords.Where(c => c.RebateType != null && c.Phone != null).ToList()
    //                        join b in totalBellRecords.Where(c => c.RebateType != null && c.Phone != null).ToList()
    //                        on new { phone = s.Phone, rebateType = s.RebateType } equals new { phone = b.Phone, rebateType = b.RebateType } into belljointable
    //                        from j in belljointable.DefaultIfEmpty()
    //                        where s.Lob == "Wireless" && j == null
    //                        select s).ToList();

    //        var wireline = (from s in totalStaplesRecords.Where(c => c.OrderNumber != null && c.SubLob != null).ToList()
    //                        join b in totalOnlyInBell.Where(c => c.OrderNumber != null && c.SubLob != null).ToList()
    //                        on new { ordernumber = s.OrderNumber, sublob = s.SubLob } equals new { ordernumber = b.OrderNumber, sublob = b.SubLob } into belljointable
    //                        from j in belljointable.DefaultIfEmpty()
    //                        where s.Lob != "Wireless" && j == null
    //                        select s).ToList();
    ...
    public async Task<IEnumerable<CompareBellStapleDto>> GetBellStapleCompareFromLocalDb(FilterItemDto filterItemDto)
    {
        ...
        {
            _logger.LogError($"GetBellStapleCompareFromLocalDb: {ex.Message}");
            return null;
        }
    }
